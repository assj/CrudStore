defmodule CrudStore.ProductControllerTest do
  use CrudStore.ConnCase
  ...
  test "checks if the report was requested successfully" do
    {:ok, product} = Product.insert_product(@valid_attrs)
    conn = get(conn, product_path(conn, :report))
    assert redirected_to(conn) == product_path(conn, :index)
  end

  test "checks if the report file was correctly created" do
    {:ok, product} = Product.insert_product(@valid_attrs)
